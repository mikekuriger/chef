...
import 'package:chef/utils/log.dart';
...
class ApiService {
  ...
  static Future<String> register(String firstName, String email, String password) async {
      ...
      if (response.statusCode == 200) {
        return "✅ Check your email to confirm your Chef account.";
      } else {
  ...
  // Fetch recipe journal (hidden = false)
  static Future<List<Recipe>> fetchRecipes() async {
    final response = await DioClient.dio.get('/api/recipes');

    if (response.statusCode == 200) {
      final List data = response.data;
      return data.map((json) => Recipe.fromJson(json)).toList();
    } else {
      throw Exception('Failed to fetch recipes: ${response.statusMessage}');
    }
  }

  // Fetch recipes for Gallery
  static Future<List<Recipe>> fetchGallery() async {
  ...
  // Fetch ALL recipe journal (hidden = true)
  static Future<List<Recipe>> fetchAllRecipes() async {
    final response = await DioClient.dio.get('/api/allrecipes');
  ...
  static Future<void> deleteRecipe(int recipeId) async {
    final response = await DioClient.dio.delete('/api/recipes/$recipeId');

    if (response.statusCode != 204) {
      throw Exception('Failed to delete recipe: ${response.statusMessage}');
    }
  }
  ...
  // Generate Image
  static Future<String> generateRecipeImage(int recipeId) async {
  ...
  static Future<Map<String, dynamic>> getRecipeNotes(int recipeId) async {
  ...
  /// Returns: { id, notes, notes_updated_at }
  static Future<Map<String, dynamic>> saveRecipeNotes({
  ...
  // Fetch recipes updated since a given ISO timestamp
  static Future<List<Recipe>> fetchRecipesUpdatedSince({
    String? updatedSinceIso,
    bool includeHidden = false,
  }) async {
    // TEMP fallback so nothing breaks until backend supports it:
    return includeHidden ? fetchAllRecipes() : fetchRecipes();
  }
  ...
  static String _absoluteUrl(String maybe) {
    ...
    final base = AppConfig.baseUrl; // e.g., "https://chef-us-west-01.zentha.me"
